...
int main() {
    ...
    ColumnVector b(m);
    for (int i = 0; i < m; ++i) {
        t.elements[i] = rand() % 100;
        b.elements[i] = rand() % 100;
        cout << t.elements[i] << " " << b.elements[i] << '\n';
    }
    n = 3;
    Matrix A(m, n + 1);
    for (int i = 0; i < m; ++i) {
    ...
    fprintf(pipe, "plot [0 : 100] [0 : 100] %lf*x**3 + %lf*x**2 + %lf*x**1 + %lf*x**0 , '-' using 1:2 with points\n", x.elements[3], x.elements[2], x.elements[1], x.elements[0]);
    for (int i = 0; i < m; ++i) {
        fprintf(pipe, "%f\t%f\n", t.elements[i], b.elements[i]);
    }
